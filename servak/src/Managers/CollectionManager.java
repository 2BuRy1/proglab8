...
public class CollectionManager {
    ...
    public void setMarines(Vector<SpaceMarine> settedmarines){
        this.marines = settedmarines;
    }


    public void loadCollection(){
        this.setMarines(dataBaseManager.createCollection());
    }
     ...
     */
    public void updateByID(Long id,  SpaceMarine spaceMarine) throws InvalidDataException, NoSuchElementException {
    ...
    public Vector<SpaceMarine> getMarines(User user) {
        return marines.stream().filter(spaceMarine -> spaceMarine.getUserLogin().equals(user.getLogin())).collect(Collectors.toCollection(Vector::new));
    }
    ...
    public SpaceMarine getById(Long id) {
        for (SpaceMarine spaceMarine1 : marines) {
            if (id.equals(spaceMarine1.getId())) {
                return spaceMarine1;
            }
        }
        return null;
     ...
     * @throws EmptyCollectionException коллекция пуста
     */
    public boolean addIfMin(SpaceMarine spaceMarine) throws InvalidDataException, EmptyCollectionException, NotMinElementException {
        if (!marines.isEmpty()) {
...
//                System.out.println("Не удалось добавить элемент, так как его поле не меньше, чем наименьшее поле у других объектов ");
    ...
    public Response maxByChapter() {
        if (!marines.isEmpty()) {
            for (int i = 0; i < marines.size() - 1; i++) {
                ...
                return new Response(marines.get(i + 1).toString());
            }
    ...
    public Response printFieldAscendingCategory() throws EmptyCollectionException {
        ...
        for(SpaceMarine element : marines){
            list.add(AstartesCategory.valueOf(element.getCategory()));
        }
